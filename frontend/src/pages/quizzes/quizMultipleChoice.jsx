...
export const QuizMultipleChoice = () => {
    ...
    const [answers, setAnswers] = useState({}); // Lưu trữ nhiều đáp án cho mỗi câu hỏi
    const maxSelections = 3; // Số lượng đáp án tối đa mà người dùng có thể chọn
    ...
    const handleAnswerChange = (questionId, selectedOption) => {
        setAnswers((prev) => {
            ...
            if (currentSelections.includes(selectedOption)) {
                ...
                // Thêm tùy chọn nếu số lượng chưa vượt quá số lượng cho phép
                return {
                    ...prev,
                    [questionId]: [...currentSelections, selectedOption]
                };
            } else {
                return prev; // Không cho phép chọn thêm
            }
        });
    };
                                            ...
                                            <ul className="mt-2 space-y-1">
                                                {question.options && question.options.length > 0 ? (
                                                    question.options.map((option, index) => (
                                                        <li
                                                            key={option.id}
                                                            className={`bg-gray-100 rounded p-2 flex items-center cursor-pointer ${answers[question.question_id] && answers[question.question_id].includes(option.answer) ? 'bg-blue-200' : ''}`}
